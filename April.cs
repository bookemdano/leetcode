...
namespace leetcode
{
    public class April
    {
        ...
        static void TestPowerOfThree()
        {
            ...
            Dessert.IsTrue(c.IsPowerOfThree(243));
            Dessert.IsTrue(!c.IsPowerOfThree(1594322));
            
            Dessert.IsTrue(c.IsPowerOfThree(9));
            Dessert.IsTrue(c.IsPowerOfThree(27));
            Dessert.IsTrue(c.IsPowerOfThree(1));
            Dessert.IsTrue(c.IsPowerOfThree(3));
            Dessert.IsTrue(!c.IsPowerOfThree(45));
            Dessert.IsTrue(!c.IsPowerOfThree(0));
            Dessert.IsTrue(!c.IsPowerOfThree(-3));
            Dessert.IsTrue(!c.IsPowerOfThree(-9));
            Dessert.IsTrue(!c.IsPowerOfThree(-27));
            Dessert.IsTrue(!c.IsPowerOfThree(-45));
        ...
        public bool IsPowerOfThreeWithLog(int n)
        {
            ...
            return n == Math.Pow(3, log);
        }

        private static readonly int[] goods = new int[] { 1, 3, 9, 27, 81, 243, 729, 2187, 6561, 19683, 59049, 177147, 531441, 1594323, 4782969, 14348907, 43046721, 129140163, 387420489, 1162261467 };
        ...
        { 
            ...
            var lines = System.IO.File.ReadAllLines("assets\\input20210426a.txt");
            var array = lines.Select(l => int.Parse(l)).ToArray();
            sw.Restart();
            Dessert.AssertSame(
                72329,
                FurthestBuilding(array,
                    926413609,
                    24790),
                $"in {sw.Elapsed}");

            lines = System.IO.File.ReadAllLines("assets\\input20210426b.txt");
            array = lines.Select(l => int.Parse(l)).ToArray();
            sw.Restart();
            Dessert.AssertSame(
                54056,
                FurthestBuilding(array,
        ...
        public int FurthestBuildingNoDelta(int[] heights, int bricks, int ladders)
        {
            ...
            for (int i = 0; i < heights.Length; i++)
            {
                if (heights[i] <= 0)
                    continue;
                sub.Add(heights[i]);
                if (!CanDo(sub, bricks, ladders))
                    return i - 1;
            }
            return heights.Length - 1;
        }

        private bool CanDo(IList<int> heights, int bricks, int ladders)
        {
            if (heights.Count() <= ladders)
                return true;
            var sum = 0L;
            var ordered = heights.OrderByDescending(h => h).Skip(ladders).ToArray();
        ...
        void AddSorted(IList<KeyValuePair<int, long>> sub, KeyValuePair<int, long> kvp)
        {
            var i = 0;
